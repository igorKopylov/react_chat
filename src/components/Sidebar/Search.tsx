...
import { setUsers } from '../../redux/sidebar/slice';
...
const Search = () => {
    ...
    return (
        <S.Container>
            <S.Loupe src={loupe} alt='magnifying glass' />
            <S.Input ref={inputRef} value={value} onChange={(event: any) => setValue(event.target.value)} placeholder='Search user... ' />
            {
                value && <svg onClick={() => clearInputValue()} width={28} height={28} style={{ opacity: 0.6, cursor: 'pointer' }} viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
